...
`getent passwd` is ran on the necessary head nodes (`tigressdata` and others) to capture all RC users (~3400). This output is combined into a master file which is stored on each head node. The master file is updated three times per day M-F. `name2id` uses the `fuzzywuzzy` package for fuzzy string matching. The top seven most likely results are presented along with their probabilities.
